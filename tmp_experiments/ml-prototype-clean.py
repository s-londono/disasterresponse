...
vctr = CountVectorizer(tokenizer=tokenize)
tfid = TfidfTransformer()

X_train_counts = vctr.fit_transform(X_train)
X_train_tfidf = tfid.fit_transform(X_train_counts)
...
clsfr = MultiOutputClassifier(RandomForestClassifier(n_estimators=10))

clsfr.fit(X_train_tfidf, Y_train)

X_test_counts = vctr.transform(X_test)
X_test_tdidf = tfid.transform(X_test_counts)

Y_pred = clsfr.predict(X_test_tdidf)
...
with open("multioutput-randomforest-clsfr.pickle", mode="wb") as pickle_file:
